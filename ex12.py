age = input("How old are you? ")
height = input("How tall are you? ")
weight = input("How much do you weigh? ")

print(f"So, you're {age} old, {height} tall and {weight} heavy.")

# Study Drills
# 1. In Terminal, where you normal run python3.6 to run your scripts, type pydoc input. Read what it says. If you're on Windows try python 3.6 -m pydoc input instead.

# 2. Get out of pydoc by typing q to quit.

# 3. Look onine for what the pydoc command does.
# '...automatically generates documentation from Python modules'
# https://docs.python.org/3/library/pydoc.html?highlight=pydoc

# 4. Use pydoc to also read about open, file, os, and sys. It's alright if you do not understand those; just read through and take notes about interesting things.